...
export default function Flip(props: {
  pageChange: (pageNumber: number, canvas: HTMLCanvasElement) => void;
}) {
  ...
  const book = useRef<HTMLDivElement>();
                ...
                <div
                  ...
                  style={{
                    zIndex:
                      (playing &&
                        ((direction === "forth" &&
                          Number(e) <= pageNumber - 2) ||
                          (direction === "back" && Number(e) <= pageNumber))) ||
                      (!playing && Number(e) < pageNumber)
                        ? Number(e)
                        : 0,
                  }}
                  className={`book__page book__page--${e} ${
                    Math.abs(pageNumber - Number(e)) > 7 ? "hidden" : ""
                  } ${Number(e) < pageNumber ? "rotated" : ""}`}
                >
                  <div onClick={toggle} className="book__page-front">
                    <canvas
                      ref={(ele) => (pageRef.current[Number(e)] = ele)}
                    ></canvas>
                  </div>
                  <div onClick={toggle} className="book__page-back">
                    <canvas
                      ref={(ele) => (pageRef.current[Number(e) + 1] = ele)}
                    ></canvas>
                  </div>
                </div>
              );
